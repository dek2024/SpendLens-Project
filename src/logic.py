...
class ExpenseParser(IParser):
    """Enhanced parser with smarter numeric + word-based amount detection (supports cents)."""
    ...
    def parse_amount(self, text: str) -> float:
        ...
        try:
            ...
            # 2️⃣ Written numbers with "dollars" or "and X cents"
            written_match = re.search(r"([\w\s-]+)\s*dollar", text_clean)
            if written_match:
                words = written_match.group(1).replace("-", " ").strip()
                value = float(w2n.word_to_num(words))

                cents_match = re.search(r"(\d+)\s*cent", text_clean)
                if cents_match:
                    cents = int(cents_match.group(1))
                    value += cents / 100.0

                logger.info("Parsed numeric amount")
                return round(value, 2)

            # 3️⃣ Fallback — spoken decimals like "six point five"
            decimal_words = re.search(r"([\w\s-]+)\s*point\s*([\w\s-]+)", text_clean)
            if decimal_words:
                whole = w2n.word_to_num(decimal_words.group(1))
                frac_str = "".join(decimal_words.group(2).split())
                try:
                    frac_val = int(frac_str)
                    decimal = float(f"{whole}.{frac_val}")
                    logger.info("Parsed numeric amount")
                    return round(decimal, 2)
                except Exception:
                    pass

            # 4️⃣ Handle standalone written numbers like "fifteen" or "twenty five"
            word_only_match = re.fullmatch(r"[a-z\s-]+", text_clean)
            if word_only_match:
                try:
                    value = float(w2n.word_to_num(text_clean.strip()))
                    logger.info("Parsed numeric amount")
                    return round(value, 2)
                except Exception:
                    pass
...
class ExcelFormatter:
    ...
    def format_workbook(self, file_path: str) -> None:
        try:
            ...
            # Add bar chart (ignore TOTAL row visually)
            chart = BarChart()
            ...
            if "Category" in df.columns and "Amount ($)" in df.columns:
                cats = Reference(ws, min_col=2, min_row=2, max_row=ws.max_row - 1)
                vals = Reference(ws, min_col=3, min_row=1, max_row=ws.max_row - 1)
